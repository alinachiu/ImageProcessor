...
/**
 * Represents a mock model for the ILayerModel that allows us to confirm that the controller is
 * passing the inputs through correctly.
 */
public class MockILayerModel implements ILayerModel {
  ...
  /**
   * Constructs the mock model for the ILayerModel using the given appendable that will append a
   * method's parameters to determine if the controller has passed the inputs correctly.
   *
  ...
  public ILayer getCurrentLayer() throws IllegalArgumentException {
    return this.delegate.getCurrentLayer();
  }
  ...
  @Override
  public void mosaicCurrent(IPhotoEffect mosaicOper, int numSeeds) throws IllegalArgumentException {
    this.delegate.mosaicCurrent(mosaicOper, numSeeds);
  }

  @Override
  public void downscaleCurrent(IImageResize downscaleOper, int width, int height)
      throws IllegalArgumentException {
    this.delegate.downscaleCurrent(downscaleOper, width, height);
  }
